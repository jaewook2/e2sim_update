...
#include <cerrno> // add for debuging by jlee at 251010
...
int sctp_start_client(const char *server_ip_str, const int server_port, const int client_port)
{
  ...
  LOG_I("[SCTP] Connection established");
    ...
    // add for configure heartbeat
  /*
  struct sctp_paddrparams paddrparams = {0};
  struct sockaddr_in addr4;
  memset(&addr4, 0, sizeof(addr4));
  addr4.sin_family = AF_INET;
  addr4.sin_port = htons(server_port);
  inet_pton(AF_INET, server_ip_str, &addr4.sin_addr);

  memcpy(&paddrparams.spp_address, &addr4, sizeof(addr4));
  LOG_I("[SCTP-Heartbeat] HeartBeat Disable");

  paddrparams.spp_flags = SPP_HB_DISABLE;
  paddrparams.spp_hbinterval = 0;
  //paddrparams.spp_flags = SPP_HB_ENABLE;
  //  paddrparams.spp_hbinterval = 5000;      // 5초
  //paddrparams.spp_pathmaxrxt = 3;

  if (setsockopt(client_fd, IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS,
                &paddrparams, sizeof(paddrparams)) < 0) {
    perror("setsockopt(SCTP_PEER_ADDR_PARAMS) failed");
  }
  */
  // 1010
...
int sctp_receive_data(int &socket_fd, sctp_buffer_t &data)
{
  ...
  //1010 update for debuggin by JLEE
  if (recv_len == -1) {
      LOG_E("[SCTP] recv failed: %s", strerror(errno));
      return -1;  
  }
  else if (recv_len == 0)
  {
