...
void encode_kpm_function_description(E2SM_KPM_RANfunction_Description_t* ranfunc_desc) {
  ...
  uint8_t* e2sm_oid = (uint8_t*)"1.3.6.1.4.1.53148.1.2.2.1";

  LOG_I("short_name: %s, func_desc: %s, e2sm_oid: %s", short_name, func_desc, e2sm_oid);
  ...
  *ranfunc_desc->ranFunction_Name.ranFunction_Instance = 1;
  ...
  ranfunc_desc->ranFunction_Name.ranFunction_E2SM_OID.buf =
      (uint8_t*)calloc(1, strlen((char*)e2sm_oid));
  memcpy(ranfunc_desc->ranFunction_Name.ranFunction_E2SM_OID.buf, e2sm_oid,
         strlen((char*)e2sm_oid));
  ranfunc_desc->ranFunction_Name.ranFunction_E2SM_OID.size = strlen((char*)e2sm_oid);
  ...
  *measDataItem->incompleteFlag = 0;
  ...
  for (size_t i = 0; i < NUMBER_MEASUREMENTS; i++)
  {
    ...
    *measLabel->noLabel = 0;
    ...
    measType.present = MeasurementType_PR_measName; // MeasurementType_PR_measID;
    uint8_t* metrics = (uint8_t *)performance_measurements[i];
